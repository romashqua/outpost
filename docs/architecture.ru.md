...
- HA-failover между шлюзами (автоматическое переключение эндпоинта при потере хендшейка)
...
| `gateways` | WireGuard-шлюзы (эндпоинт, публичный ключ, хэш токена, последнее обнаружение, health_status, consecutive_failures) |
...
## Высокая доступность шлюзов (Gateway HA)

Outpost поддерживает мульти-шлюзовые развёртывания для устранения единых точек отказа. Несколько шлюзов могут обслуживать одну сеть, и клиенты автоматически переключаются при недоступности шлюза.
...
```
       ...
       │при: 100 │  │при: 90  │  │при: 80   │
       │healthy  │  │healthy  │  │unhealthy │
       └────┬────┘  └────┬────┘  └──────────┘
            │             │
     VPN-клиенты автоматически
     переключаются с gateway-1
     на gateway-2 если нет
     хендшейка 45с
```

**Поток мониторинга здоровья:**
1. Каждый шлюз отправляет heartbeat каждые 30 секунд через gRPC-стрим
2. Core обновляет `last_seen`, устанавливает `health_status = 'healthy'`, сбрасывает `consecutive_failures = 0`
3. Горутина `MonitorGatewayHealth()` запускается каждые 60 секунд, проверяет устаревшие шлюзы (`last_seen > 90с`)
4. Устаревшие шлюзы получают `consecutive_failures++`; после 3 отказов → `health_status = 'unhealthy'`
5. При переходе в unhealthy core отправляет FullResync на peer-шлюзы той же сети

**Поток клиентского failover:**
1. Клиент получает упорядоченный список шлюзов при регистрации (по приоритету DESC)
2. Health-тикер (15с) проверяет время последнего WireGuard-хендшейка
3. Если нет хендшейка >45с (3 последовательных проверки), клиент переключает peer endpoint на следующий шлюз
4. WireGuard peer переконфигурируется на месте без разрыва TUN-интерфейса
...
## Горизонтальное масштабирование (Multi-Core)

Outpost спроектирован для работы нескольких экземпляров `outpost-core` за балансировщиком нагрузки — как для производительности, так и для отказоустойчивости. Никакого etcd или внешней системы консенсуса не требуется — PostgreSQL и Redis (уже в стеке) обеспечивают всю необходимую координацию.

### Что уже масштабируется без изменений

| Компонент | Почему масштабируется |
|-----------|----------------------|
| HTTP API | Полностью stateless — всё состояние в PostgreSQL. Любой core может обработать любой запрос. |
| Шлюзы | Уже переподключаются с экспоненциальным backoff при потере gRPC-стрима. Если core падает — шлюз переподключается к другому. |
| Сессии | Уже хранятся в Redis (общие для всех core). |
| JWT-аутентификация | Симметричный HMAC-SHA256 — любой core с тем же `OUTPOST_JWT_SECRET` валидирует токены. |
| Фронтенд | Статический SPA через `go:embed` — никакого серверного состояния. |
| База данных | PostgreSQL — единственный источник истины. Пул соединений pgx — per-core. |

### Что требует Redis Pub/Sub (одно изменение)

`StreamHub` хранит gRPC-стримы в памяти. Когда Core-1 одобряет устройство, только шлюзы, подключённые к Core-1, получают `PeerUpdate`. Шлюзы Core-2 не узнают об этом до следующего периодического FullResync.

**Решение:** При отправке обновления пиров/файрвола/конфигурации core также публикует событие в Redis Pub/Sub канал (`outpost:events`). Другие core подписаны на этот канал и пересылают событие своим локально подключённым шлюзам.

```
Админ одобряет устройство на Core-1
    → Core-1 записывает в PostgreSQL
    → Core-1 отправляет PeerUpdate своим шлюзам (StreamHub)
    → Core-1 публикует событие в Redis "outpost:events"
    → Core-2 получает событие из Redis
    → Core-2 отправляет PeerUpdate своим шлюзам (StreamHub)
```

Если сообщение Redis Pub/Sub потеряется (Redis работает в режиме fire-and-forget), существующий периодический FullResync обеспечивает eventual consistency. Это даёт strong consistency в обычном случае и eventual consistency как фолбэк.

### Singleton-задачи (выбор лидера)

Некоторые горутины должны работать ровно на одном core, чтобы избежать дублирования:

| Задача | Сейчас | Решение для multi-core |
|--------|--------|----------------------|
| `MonitorGatewayHealth` | На единственном core | `pg_try_advisory_lock(1)` — только один core захватывает |
| `managePeerStatsPartitions` | На единственном core | `pg_try_advisory_lock(2)` — только один core захватывает |
| Очистка token blacklist | На единственном core | `pg_try_advisory_lock(3)` — только один core захватывает |

PostgreSQL advisory locks освобождаются автоматически при разрыве соединения, поэтому если leader core падает, другой core захватывает лок на следующем тике.

### Rate limiting

In-memory `ipRateLimiter` нужно перевести на Redis (`INCR` + `EXPIRE`), чтобы лимиты применялись глобально для всех core, а не per-core.

### Топология развёртывания

```
                    Балансировщик нагрузки (L4 для gRPC, L7 для HTTP)
                         │
              ┌──────────┼──────────┐
              │          │          │
         ┌────▼───┐ ┌────▼───┐ ┌────▼───┐
...
**Минимум (dev/тестирование):** 1 core, 1 gateway, 1 PG, 1 Redis — работает как есть, без изменений.

**HA (продакшен):** 2–3 core за балансировщиком, 2+ шлюза на сеть, PostgreSQL со стриминг-репликой, Redis Sentinel.

**Требования к балансировщику:**
- HTTP: стандартный L7 (nginx, HAProxy, облачный ALB)
- gRPC: L4 или gRPC-aware L7 (envoy, nginx с `grpc_pass`, облачный NLB)
- gRPC-соединения шлюзов долгоживущие; используйте балансировку по соединениям (L4), а не per-request

### Почему не etcd / NATS / Kafka?

| Вариант | Решение | Причина |
|---------|---------|---------|
| etcd | Избыточно | 3+ узла для кворума, TLS-сертификаты, compaction — огромная операционная нагрузка для наших задач |
| NATS | Не нужно | Добавляет новую зависимость; Redis Pub/Sub покрывает наш use case |
| Kafka | Сильно избыточно | Нам нужна fire-and-forget нотификация, а не durable streaming |
| Redis Pub/Sub | **Выбран** | Уже развёрнут, ноль новых зависимостей, просто, достаточно с фолбэком на FullResync |
| PostgreSQL LISTEN/NOTIFY | Альтернатива | Работает, но лимит payload 8КБ; Redis гибче и уже обслуживает сессии |

### TLS для gRPC

В продакшен multi-core развёртываниях gRPC-трафик между шлюзами и core должен использовать TLS:
...
- `OUTPOST_GRPC_TLS_CERT` — путь к TLS-сертификату core
- `OUTPOST_GRPC_TLS_KEY` — путь к приватному ключу TLS core
- `OUTPOST_GRPC_TLS_CA` — путь к CA-сертификату (для верификации шлюза)
- `OUTPOST_GATEWAY_TLS_CERT` — сертификат шлюза
- `OUTPOST_GATEWAY_TLS_KEY` — приватный ключ шлюза
- `OUTPOST_GATEWAY_TLS_CA` — CA-сертификат (для верификации core)

При установке TLS env vars gRPC-сервер использует `credentials.NewTLS()` вместо `grpc.Creds(insecure)`. Шлюзы верифицируют сертификат core по CA. Поддерживается mutual TLS (mTLS) — core верифицирует сертификаты шлюзов при установленном `OUTPOST_GRPC_TLS_CA`.
